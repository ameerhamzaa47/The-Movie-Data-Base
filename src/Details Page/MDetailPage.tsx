...
import { toggleMovieInList, isMovieInList } from '../Auth/Firebase'; // Import the helper functions
...
const MDetailPage: FC = () => {
  ...
  useEffect(() => {
    if (id && uid) {
      // Fetch movie data and check if it's in user's lists
      fetchMovieData(id);
    }
  }, [id, uid]);
  ...
  const fetchMovieData = async (id: string) => {
    try {
      ...
      if (foundMovie) {
        ...
        // Check if the movie is in any list
        if (uid) {
          checkIfInList(foundMovie.id.toString(), 'list');
          checkIfInList(foundMovie.id.toString(), 'favorites');
          checkIfInList(foundMovie.id.toString(), 'watchlist');
        }
      }
    } catch (error) {
  ...
  const checkIfInList = async (movieId: string, listType: string) => {
    if (uid) {
      const isIn = await isMovieInList(uid, movieId, listType);
      if (listType === 'list') setIsInList(isIn);
      if (listType === 'favorites') setIsInFavorites(isIn);
      if (listType === 'watchlist') setIsInWatchlist(isIn);
    }
  };

  const handleIconClick = async (listType: string) => {
    if (uid && movies.length > 0) {
      const movieId = movies[0].id.toString();
      await toggleMovieInList(uid, movieId, listType);

      // After toggling, update the local state to reflect the change
      if (listType === 'list') setIsInList(!isInList);
      if (listType === 'favorites') setIsInFavorites(!isInFavorites);
        ...
        <div className='flex flex-col md:flex-row justify-between relative' style={{ backgroundImage: `url(${movies[0].imageUrl})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', }}>
          ...
          <div className='w-3/4 md:w-2/3 text-white mx-10 my-10 relative'>
            ...
            <button onClick={() => handleIconClick('list')}
              className={`icon-button ${isInList ? 'bg-red-500' : 'bg-[#032541]'} rounded-full mx-2`}>
              <ListBulletIcon className="h-10 w-10 p-2 text-white" />
            </button>
            <button onClick={() => handleIconClick('favorites')} data-tooltip-id="my-tooltip" data-tooltip-content="Mark as favorite" className={`icon-button ${isInFavorites ? 'bg-red-500' : 'bg-[#032541]'} rounded-full mx-2`}>
              <HeartIcon className="h-10 w-10 p-2 text-white" />
            </button>
            <button onClick={() => handleIconClick('watchlist')} data-tooltip-id="my-tooltip" data-tooltip-content="Add to your watchlist" className={`icon-button ${isInWatchlist ? 'bg-red-500' : 'bg-[#032541]'} rounded-full mx-2`}>
              <BookmarkIcon className="h-10 w-10 p-2 text-white ${isInWatchlist ? 'bg-red-500' : 'bg-[#032541]'} rounded-full mx-2`}" />
            </button>
...
//             </div>
//             {/* button */}
//             <button data-tooltip-id="my-tooltip" data-tooltip-content="Add to List" className="icon-button bg-[#032541] rounded-full mx-2">
//               <ListBulletIcon className="h-10 w-10 p-2 text-white" />
//             </button>
//             <button data-tooltip-id="my-tooltip" data-tooltip-content="Mark as favorite" className="icon-button bg-[#032541] rounded-full mx-2">
//               <HeartIcon className="h-10 w-10 p-2 text-white" />
//             </button>
//             <button data-tooltip-id="my-tooltip" data-tooltip-content="Add to your watchlist" className="icon-button bg-[#032541] rounded-full mx-2">
