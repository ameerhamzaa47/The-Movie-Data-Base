...
const UpdateMovie: FC = () => {
  ...
  const onSubmit = async (data: any) => {
    let updatedData: UpdateData = {
      ...
      genre: data.genre.split(',').map((genre: string) => genre.trim()),
      score: data.score,
      videoUrl: data.Video,
      movieUrl: data.movieUrl,
      overview: data.overview,
      imageUrl: movieData?.imageUrl, // Keep the old image URL if no new image is selected
    };
  
    if (imageFile) {
      const formData = new FormData();
      formData.append('file', imageFile);
      formData.append('upload_preset', 'tvShows');  // Make sure this matches your Cloudinary preset
  
      try {
        // Upload the image to Cloudinary
        const uploadResponse = await fetch('https://api.cloudinary.com/v1_1/dlc8zsx7m/upload', {
          method: 'POST',
          body: formData,
        });
        ...
        if (!uploadResponse.ok) {
          ...
          alert(`Image upload failed: ${errorData.error.message}`);
          return;  // Exit function if upload failed
        }
  
        const imageData = await uploadResponse.json();
        const imageUrl = imageData.secure_url;  // Get the URL of the uploaded image
  
        // Update the movie data with the new image URL
        updatedData.imageUrl = imageUrl;
  
        // Now send the PUT request to update the movie
      ...
      // If no image selected, update movie without the image
      const response = await fetch(`http://localhost:5000/Movies/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      <form className='mt-5' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='flex flex-col my-2'>
          ...
          <input
            ...
            type="text"
            defaultValue={movieData?.genre.join(', ')} // Join the genre array as a string
          />
        </div>
